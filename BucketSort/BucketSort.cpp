...
// DONE: Put the item in the appropraite bucket.
void ManyBuckets::addItem(unsigned long item) {
	//handle 0 case, what is it though
	if (numBuckets == 1) {
		arr[0].push_back(item);
		return;
	}
	arr[(item / (ULONGMAX / numBuckets + 1))].push_back(item);
}

// DONE:  For this function, loop over every item and call addItem
...
void sortEachBucketMultithreaded() {
	...
	do {
		myMutex.lock();
		myBlock = currentWorkUnit;
		currentWorkUnit++;
		myMutex.unlock();
		if (myBlock >= numBuckets) { return; }
		
		recQuickSort(globalBuckets->getBucket(myBlock), 0, globalBuckets->getNumItemsInABucket(myBlock));
...
void combineBuckets() {
	int counter = 0;
	for (unsigned int i = 0; i < numBuckets; i++) {
		for (unsigned int j = 0; j < globalBuckets->getBucket(i).size(); j++) {
			list[counter] = globalBuckets->getBucket(i)[j];
			counter++;
		}
	}
}
...
void bucketSort(bool displayOutput, bool useMultiThreading) {
	...
	if (!useMultiThreading) {
		...
		fork
		join
		detele thread tracking objects
		*/

		currentWorkUnit = 0;
		thread* myThreads = new thread[numThreadsToUse]; // myThreads simply tracks
