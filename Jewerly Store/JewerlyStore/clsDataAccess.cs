...
public class clsDataAccess
{
    ...
    {
        _sConnectionString = @"Provider=Microsoft.Jet.OLEDB.4.0;Data source= " + Directory.GetCurrentDirectory() + "\\Invoice.mdb";
    }
        ...
        try
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(_sConnectionString))
            {
                using (OleDbDataAdapter adapter = new OleDbDataAdapter())
                {
                    ...
                    adapter.SelectCommand = new OleDbCommand(sSQL, conn) { CommandTimeout = 0 };
    ...
    public int ExecuteNonQuery(string sSQL)
    {
        try
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(_sConnectionString))
            {
                ...
                //Add the information for the SelectCommand using the SQL statement and the connection object
                OleDbCommand cmd = new OleDbCommand(sSQL, conn) { CommandTimeout = 0 };
